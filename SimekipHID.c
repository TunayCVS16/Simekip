...
/* Simekip HID (ATmega16U2)
   - Pure HID joystick (no REPORT_ID), 64 buttons + 2 hat + 8 axes
   - Fills zeros; real data can be fed from the main MCU over UART (extend as needed)
*/
...
void EVENT_USB_Device_StartOfFrame(void) { HID_Device_MillisecondElapsed(&HID_Interface); }
                                    ...
                                    const void** const DescriptorAddress)
{
  ...
  static const USB_Descriptor_Device_t PROGMEM DeviceDescriptor =
  {
    ...
    .VendorID               = 0x03EB,          /* Test VID (Atmel/Microchip) */
    .ProductID              = 0x2043,          /* Generic HID Joystick PID */
  ...
  else if (DescriptorType == DTYPE_Configuration)
  {
    ...
    C->Config.Header.Size                 = sizeof(USB_Descriptor_Configuration_t_Impl);
    C->Config.Header.Type                 = DTYPE_Configuration;
    C->Config.TotalConfigurationSize      = sizeof(USB_Descriptor_Configuration_t_Impl);
    ...
    *DescriptorAddress = C;
    return sizeof(USB_Descriptor_Configuration_t_Impl);
  }
